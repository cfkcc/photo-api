...
@Service("authQQ")
public class AuthQQService implements AuthService {
...
*/
	public static final String appId=AuthConfig.getString("auth.qq.appId");
	public static final String appKey=AuthConfig.getString("auth.qq.appKey");
	
	public static final String authUrl=AuthConfig.getString("auth.qq.authUrl");
	
	public static final String meUrl=AuthConfig.getString("auth.qq.meUrl");
	
	public static final String callback=AuthConfig.getString("auth.qq.callback");
	...
	public static void main(String []args) throws UnsupportedEncodingException{
		...
		/*String xx = "callback( {\"client_id\":\"YOUR_APPID\",\"openid\":\"YOUR_OPENID\",\"cli\":\"xx\"} );";
		String regex = "\\s*\"openid\":\"(.*?)\"\\s*";
		Pattern pattern =Pattern.compile(regex);
		Matcher mat = pattern.matcher(xx);
		if(mat.find()){
	...
	public boolean auth(String oid, String accessToken, String sysType,String packagename) {
		String result = HttpUtil.getHtml(meUrl, "utf-8","access_token="+accessToken);
		if(StringUtils.isNotBlank(result)){
			Matcher mat =Pattern.compile("\\s*\"openid\":\"(.*?)\"\\s*").matcher(result);
			if(mat.find()){
				String oidName = mat.group(1);
				if(oidName.equals(oid)){
					return true;
				}
			}
		}
